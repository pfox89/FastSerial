...
namespace SerialXP
{
  ...
  public:
    ...
    int waitEvent() noexcept
    {
      while (0 == (_events & EV_TXEMPTY))
      {
        if (WaitCommEvent(_hPort, &_events, nullptr))
        {
          continue;
        }
        else
    ...
    bool transmitComplete() const noexcept { return (_events& EV_TXEMPTY) == EV_TXEMPTY;  }
