...
struct SerialEnumImpl
{
  ...
  int init(unsigned int type) noexcept
  {
    ...
    {
      case SerialBusType::BUS_USB: stat = udev_enumerate_add_match_property(_enumerate.get(), "ID_BUS", "usb"); break;
      case SerialBusType::BUS_PCI: stat = udev_enumerate_add_match_property(_enumerate.get(), "ID_BUS", "pci"); break;
      default:
  ...
  int getInfo(SerialDeviceInfo& info, const char* path) noexcept
  {
    ...
    if (bus == nullptr)
    {
      ...
      if ((parent = udev_device_get_parent_with_subsystem_devtype(_dev.get(), "pnp", nullptr)) != nullptr)
      {
        ...
        {
          if (info.description[6] == '1') info.description = "16550A-compatible PNP Serial Device";
          else
      ...
      else if ((parent = udev_device_get_parent_with_subsystem_devtype(_dev.get(), "platform", nullptr)) != nullptr)
      {
        ...
        if (strcmp(info.description, "serial8250") == 0) info.description = "8250/16550 Series Platform Serial Device";
        else
    ...
    else if (strncmp(bus, "usb", 3) == 0)
    {
      ...
      if (
        info.description == nullptr || (strcasestr(info.description, "controller") != nullptr) ||
        (strcasestr(info.description, "hub") !=
         nullptr) // Workaround for funky udev bug where it shows the parent device model
      )
      {
  ...
  int next(SerialDeviceInfo& info) noexcept
  {
    ...
    do {
      do {
        if (_devices == nullptr) _devices = udev_enumerate_get_list_entry(_enumerate.get());
        else
        {
      ...
      while (phypath != nullptr)
      {
        ...
        if (strncmp(phypath, "pci", 3) == 0)
        {
          ...
          if (phypath != nullptr)
          {
            ...
            {
              if (*phypath != '0') *dest++ = std::toupper(*phypath++);
              else
                ++phypath;
            }
        ...
        else if (strncmp(phypath, "usb", 3) == 0)
        {
          ...
          while (phypath_temp > phypath && *phypath_temp != '/') { --phypath_temp; }
