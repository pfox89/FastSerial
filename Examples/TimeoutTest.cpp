...
int main(int argc, char** argv)
{
  ...
  if (status == 0)
  {
    ...
    {
      std::cerr << "Error receiving bytes " << sizeof(testpattern)-1 << " bytes sent " << total_read << " bytes received" << std::endl;
      return -5;
    }
    else
    {
      ...
      {
        std::cerr << "Took too long to time out! Expected " << timeout + 50  << "ms, took " << ms_int << "ms" << std::endl;
        return -6;
      }
      std::cout << "Successfully timeout out after reading " << total_read << " bytes in " << ms_int << "ms" << std::endl;
    }
  }
  else if(status < 0)
  {
